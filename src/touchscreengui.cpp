	...
	if (event.TouchInput.Event == ETIE_PRESSED_DOWN) {
		...
		if (m_move_id != -1) {
			if ((event.TouchInput.ID == m_move_id) &&
				...
				v2s32 old_pos = getPointerPos(event.TouchInput.ID);
				...
				if ((distance > g_settings->getU16("touchscreen_threshold")) ||
					...
					s32 dx = X - getPointerPos(event.TouchInput.ID).X;
					s32 dy = Y - getPointerPos(event.TouchInput.ID).Y;
...
inline v2s32 TouchScreenGUI::getPointerPos(size_t ID)
{
